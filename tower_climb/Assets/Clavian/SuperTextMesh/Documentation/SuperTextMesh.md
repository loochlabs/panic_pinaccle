...
<br>
Super Text Mesh Documentation - v1.7
...
[Notes & FAQ](#FAQ)
[ChangeLog & Update Warnings](#ChangeLog)
[Credits](#Credits)
...
* ###[READ ONLY] string SuperTextMesh.leftoverText
	If text is given a vertical limit and set to "Cut Off" mode, all text that gets cut off will end up here as unformatted text, ready to be sent to another Super Text Mesh object. To avoid spaces at the start, you can set the text to be leftoverText.TrimStart().
...
* ### Should I buy Super Text Mesh on itch.io or the unity asset store?
	...
	* Can update the asset through Unity without needing to drag in the .unitypackage file.
	* You can leave me a review on the asset store!
...
* On UI text, using anchor points besides the top anchor point while text is being limited vertically can have unintended effects at the moment.
...
* Backspacing. Can already be done with Unread(), just not up to a certain point yet.
...
* Make it so no error returned when font is null, even though it should never be null.
* Nice rag text.
* Each line should drop down by the size of the tallest character in its row, not the first character.
...
* "Disable animated text" now makes waves be ignored entirely instead of freezing in place.
...
* Removed lineCountLimit and replaced it with verticalLimit. In UI mode, this value can be determinded by the rect transform's height if "Limit Text" is true.
* Added new modes for how text will be treated if it goes over this limit, "Show Last", "Cut Off" and "Ignore".
* Hid debug mode from the inspector.
* Added support for pagination! If text has a limited vertically and set to "Cut Off", all the remaining text will appear in a string named "leftoverText". This string will have all tags that came before it, and all raw text that comes after the cutoff point, so it's ready to be put in another Super Text Mesh object.
...
* Init() and UnInit() are only called under OnEnable() and OnDisable() now.
...
* Split SuperTextMeshData and STMTextInfo into their own scripts.
* AutoVoice string is initialized properly, now.
